...
#include <math.h>
#include <sys/time.h>
#include "cuda_functions_no_shared_memory.h"
...
int main(int argc,char **argv)
{
  ...
  */
  ...
  //FIRST STEP INITIALIZE Y = X 
  //RUN SO MANY BLOCKS AS THE ELEMENTS
  //AND THREADS PER BLOCK ( argv[3] )
  set_y<<<(N + M-1) / M,M>>>(y,x,N);
  int iteration=0;
  /*
     in parallel we are doing:
     clearing the y_new
     the formula : running 1 block for each element and 1 thread per block
     updating the m array
     updating the y array

     but we are doing sequentially the frobenius norm
     at the end checking the forbenius norm
     ...
     norm_of_m : we do it sequentially
   
     calculate : elements blocks and 1 thread per block
  ...
  */
  while(error_cpu[0]>EPSILON)
  {
    ...
    calculate<<<elements,1>>>(x_p,y_p,y_n_p,elements,dimensions,N);
    update_m<<<(N + M-1) / M,M>>>(y_n,y,m,N);
    update<<<(N + M-1) / M,M>>>(y_n,y,N);
    norm_of_m<<<1,1>>>(m,elements,dimensions,error_gpu);
    //TRANSFER THE RESULT TO HOST AND CHECK IT BEFORE GETTING TO LOOP
  ...
  printf("\nmean shift CUDA without Shared Memory time = %f\n\n", seq_time);
...
void check_input(int argc,char **argv)
{
  ...
  {
    printf("Dimensions must be in range [1,%d]\n",MAX_DIMENSIONS);
    exit(1);
  }
  if(threads>dimensions||threads<=0)
  {
    printf("Threads per block must be in range [1,%d]\n",dimensions);
    exit(1);
  }
}
...
void create_output_file(void)
{
  double temp;
  int write_value;
  FILE *fp=fopen("result_without_shared_memory.bin","wb");
  if(fp==NULL)
  {
    printf("Error opening the file : result_without_shared_memory.bin\n");
    exit(1);
  }
  for(int i=0;i<elements;i++)
  {
    for(int j=0;j<dimensions;j++)
    {
      ...
      if(write_value!=1)
      {
        printf("error writing in file : result_without_shared_memory.bin\n");
        exit(1);
      }
    }
  }
...
void test_with_matlab()
{
  ...
  c=fopen("result_without_shared_memory.bin","rb");
  if(c==NULL)
  { 
    printf("error opening the file : result_without_shared_memory.bin\n");
    exit(1);
  ...
  {
    printf("result_without_shared_memory.bin and result_matlab.bin Do not have the same size\n");
    exit(1);
  }
  rewind(c);
  rewind(matlab);
...
void test_with_sequential()
{
  ...
  if(c_seq==NULL)
  { 
    printf("error reading from file : result_sequential.bin\n");
    exit(1);
  }
  c_cuda=fopen("result_without_shared_memory.bin","rb");
  if(c_cuda==NULL) 
  {
    printf("error reading from file : result_without_shared_memory.bin\n");
    exit(1);
  }
  ...
  {
    printf("result_sequential.bin and result_without_shared_memory.bin Do not have the same size\n");
    exit(1);
  }
  rewind(c_seq);
  rewind(c_cuda);
